...
ImGuiLog::ImGuiLog():m_rep(new Rep()) {
}
...
void ImGuiLog::AddLog(const char* fmt, ...) {
	int old_size = m_rep->Buf.size();
	va_list args;
	va_start(args, fmt);
	m_rep->Buf.appendfv(fmt, args);
	va_end(args);
	for (int new_size = m_rep->Buf.size(); old_size < new_size; old_size++) {
		if (m_rep->Buf[old_size] == '\n') m_rep->LineOffsets.push_back(old_size + 1);
	}
}

void ImGuiLog::Draw(const char* title, bool* p_open) {
	m_rep->Draw(title,p_open);
}
