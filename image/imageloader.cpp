...
Image* loadImageR32f(CString path) {
	...
	auto data = static_cast<short*>(malloc(width * 4 * height));
...
Image* loadImage(CString path) {
	...
	ImageFormat formats[] = {{},ImageFormat::alpha8, ImageFormat::intensityAlpha16, ImageFormat::rgb24, ImageFormat::rgba32};
